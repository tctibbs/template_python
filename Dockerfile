...
# a container image, so disable this to reduce the size of images.
...
# Default command to run Python REPL. Override this in docker run or docker-compose.yml
# Example: docker run --rm <image-name> python -c "from python_template import add; print(add(1, 2))"
CMD ["python"]
